...
function xmlNodeToItem(child, tag, newLine, styleList = null) {
  ...
  if (child.nodeName === "span") {
    if (child.hasAttribute("style") === true) {
      ...
      isBlue = availableStyles.blue
        ? styles.indexOf(`color: ${availableStyles.blue.color};`) > -1
        : false;
      isRed = availableStyles.red
        ? styles.indexOf(`color: ${availableStyles.red.color};`) > -1
        : false;
      isGreen = availableStyles.green
        ? styles.indexOf(`color: ${availableStyles.green.color};`) > -1
        : false;
      isBlueMarker = availableStyles.blue_hl
        ? styles.indexOf(
            `background-color: ${availableStyles.blue_hl.backgroundColor};`
          ) > -1
        : false;
      isGreenMarker = availableStyles.green_hl
        ? styles.indexOf(
            `background-color: ${availableStyles.green_hl.backgroundColor};`
          ) > -1
        : false;
      isPinkMarker = availableStyles.pink_hl
        ? styles.indexOf(
            `background-color: ${availableStyles.pink_hl.backgroundColor};`
          ) > -1
        : false;
      isPurpleMarker = availableStyles.purple_hl
        ? styles.indexOf(
            `background-color: ${availableStyles.purple_hl.backgroundColor};`
          ) > -1
        : false;
      isYellowMarker = availableStyles.yellow_hl
        ? styles.indexOf(
            `background-color: ${availableStyles.yellow_hl.backgroundColor};`
          ) > -1
        : false;
      isOrangeMarker = availableStyles.orange_hl
        ? styles.indexOf(
            `background-color: ${availableStyles.orange_hl.backgroundColor};`
          ) > -1
        : false;
